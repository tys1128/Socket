...
private:
	...
	void handle_receive(std::size_t length)
	{
		...
		if (true && is && (icmp_hdr.type() == icmp_header::echo_reply) && (icmp_hdr.identifier() == get_identifier()) && (icmp_hdr.sequence_number() == sequence_number_))
		{
			...
			std::cout << length - ipv4_hdr.header_length()
				...
				<< std::endl;
			std::cout.write(
				static_cast<const char *>(reply_buffer_.data().data()),
				length
			);
			std::cout << std::endl;
		}
